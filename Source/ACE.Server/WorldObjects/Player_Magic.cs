    ...
    partial class Player
    {
        ...
        public void DoSpellWords(Spell spell, bool isWeaponSpell)
        {
            ...
            if (!string.IsNullOrWhiteSpace(spellWords) && !isWeaponSpell)
                EnqueueBroadcast(new GameMessageHearSpeech(spellWords, GetNameWithSuffix(), Guid.Full, ChatMessageType.Spellcasting),
                    LocalBroadcastRange, ChatMessageType.Spellcasting, new GameMessageHearSpeech("WarDetect", GetNameWithSuffix(), Guid.Full, ChatMessageType.Spellcasting), true);
        }
